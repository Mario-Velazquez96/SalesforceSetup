...
2. **The BlueSky org has NO OrgWideEmailAddress.** `SELECT ... FROM OrgWideEmailAddress` returns 0 rows. As of 2026-06-15 (client chose "Option A" send-as-owner), SequenceEmailService sets the From by matching the Target OWNER's email to a verified OWE, then falls back to a default OWE (by DisplayName), then the running user — all resolved at runtime, never hardcoded (R3/R14). With no OWEs in the sandbox it falls back to the running user. Operational prereq for real send-as-owner: each rep's email must be created + VERIFIED as an OWE. Note: `OrgWideEmailAddress` is NOT insertable in Apex tests — the owner→OWE matching is unit-tested via a `@TestVisible` injectable map seam. Deliverability still needs blueskyadvisory.com DKIM (DNS at GoDaddy; Google Workspace email; DMARC is already p=reject so DKIM is mandatory).
