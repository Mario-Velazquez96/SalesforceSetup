...
- [x] Revert the 4 previously-added `Status__c` picklist values (`Converted`, `Meeting Booked`, `Do Not Contact`, `Replied`); map `Sequence_Terminal_Status__mdt` terminal rows to the EXISTING values `Closed`, `Target Not Interested`, `Client Not Interested`, `Conflicted` (R8)
...
- [x] Extend `TargetTriggerHandlerTest`: each terminal status (`Closed`, `Target Not Interested`, `Client Not Interested`, `Conflicted`) stops + writes reason, no extra DML (R1, R2)
...
- **R1:** active target, set `Status__c='Closed'` → `Sequence_Active__c=false`,
  `Sequence_Stop_Reason__c` matching the CMDT row, `Next_Action_Date__c=null`; assert no
  added DML for the stamp (before-save).
...
- **R8:** the terminal values (`Closed`, `Target Not Interested`, `Client Not Interested`,
  `Conflicted`) already exist in the `Target__c.Status__c` value set, so the R1 saves above
  succeed; no new picklist values are added.
